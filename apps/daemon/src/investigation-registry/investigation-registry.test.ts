...
function input(goal: string, harnessKinds?: AgentHarnessKind[]) {
...
describe("InvestigationRegistry", () => {
    it("runs every investigation it takes on at the same time", async () => {
        ...
        const first = await registry.create(input("Chase the first lead"));
        const second = await registry.create(input("Chase the second lead"));
    ...
    it("gives each investigation the roster it was created with", async () => {
        ...
        const pinned = await registry.create(input("Run on one harness", [AgentHarnessKind.GLM]));
        const defaulted = await registry.create(input("Run on the default rotation"));
    ...
    it("reopens a running investigation on restart and leaves a settled one asleep", async () => {
        ...
        const working = await first.create(input("Keep working after the restart"));
        const settled = await first.create(input("Stay asleep after the restart"));
    ...
    it("summarises what an operator chooses between", async () => {
        ...
        const held = await registry.create(input("Summarise this investigation"));
    ...
    it("discards the run directory and the record when an investigation is removed", async () => {
        ...
        const held = await registry.create(input("Discard this investigation"));
    ...
    it("tells a subscriber the roster moved without being asked", async () => {
        ...
        const held = await registry.create(input("Publish the roster"));
        ...
        unsubscribe();
        await registry.create(input("Nobody is listening any more"));
