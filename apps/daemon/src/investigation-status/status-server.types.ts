...
export interface StatusServerOptions {
    ...
    /** The same readings the dispatch gate gets, so an operator sees what the investigation decided on. */
    subscriptions?: SubscriptionAllowanceReadings;
    dashboardRoot?: string;
    logLevel?: DaemonLogLevel;
    /** Both controls give up the cycle in flight; only the state the investigation settles in differs. */
