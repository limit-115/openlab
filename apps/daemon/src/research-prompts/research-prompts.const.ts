...
export const MATERIAL_ARTIFACT_POLICY =
    "An artifact is a file your own execution wrote: a measurement, a captured output, a log, a dataset. Prose, JSON you typed yourself, and a summary of what you believe happened are not artifacts. The daemon copies every path you declare into storage you do not control, re-reads and hashes the copy, and judges that copy with a precommitted evaluator you can neither see nor run, so an artifact that merely asserts a conclusion fails against an evaluator that measures one. Produce your artifacts inside the current workspace." as const;
