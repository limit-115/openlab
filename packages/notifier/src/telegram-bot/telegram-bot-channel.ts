...
export class TelegramBotChannel implements NotificationChannel {
    ...
    async deliver(message: NotificationMessage, signal?: AbortSignal): Promise<void> {
     ...
     */
    async #read(answer: Response): Promise<TelegramApiResponse> {
        try {
            return (await answer.json()) as TelegramApiResponse;
        } catch (error) {
            throw new NotificationDeliveryError(
                this.kind,
