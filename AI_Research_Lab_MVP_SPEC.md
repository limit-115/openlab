...
- Lab сама устанавливает любое нужное ей software: библиотеки, пакеты, CLI, приложения, runtimes, MCP-серверы и плагины. Отсутствие программы никогда не является блокером.
- Если не хватает credential, аккаунта, приватных данных, оборудования или разрешения, lab создаёт `CapabilityRequest` с `resource_class` и продолжает остальные направления.
...
Если не хватает ресурса, который может выдать только оператор, создаётся:

```json
{
  "type": "capability_request",
  "need": "Что именно требуется",
  "resource_class": "credential | account | private_data | hardware | authorization",
  "reason": "Какую гипотезу или experiment это разблокирует",
  "provisioning_hint": "Как команда может это предоставить"
}
```

Это запрос ресурса, а не запрос разрешения. Software, которое lab может установить сама, не имеет
`resource_class` и не запрашивается.
...
8. Missing access, который может выдать только оператор, создаёт `CapabilityRequest`, а не отказ от направления; недостающее software lab устанавливает сама.
